...
/**
 * The clinician's signing key, which the build sheet puts in the clinician's
 * hands: signed with the clinician key, held by the handset, verified by
 * the branch". Held here means held — the key is generated inside the phone's
 ...
 * key goes onto the clinician's own record once, in X.509/SPKI form, so the
 * branch and the state node verify a visit, a dose or a disposal without a key
 * exchange, years after the visit if a reviewer asks.
 */
object Keys {
    private const val ALIAS = "clinician-signing"
    ...
    private fun generate(strongBox: Boolean) {
        ...
        val spec = KeyGenParameterSpec.Builder(ALIAS, KeyProperties.PURPOSE_SIGN)
            ...
            .setUserAuthenticationRequired(false) // a clinician signs at a bedside, gloves on
