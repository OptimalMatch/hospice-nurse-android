...
# Android runs an app's own native-library files, and packaging requires the
# lib*.so name.
#   libunidatum.so   the engine (android/arm64 release archive)
...
# Usage: tools/fetch-natives.sh [unidatum version, default v2.369.0]
set -euo pipefail
V="${1:-v2.369.0}"; REPO="${UNIDATUM_REPO:-OptimalMatch/peer-to-peer-db}"
DUCKDB_VERSION="${DUCKDB_VERSION:-v1.5.5}"; ALPINE="${ALPINE:-v3.21}"
LOCAL_DIST="${LOCAL_DIST:-$HOME/github/peer-to-peer-db/dist}"
cd "$(dirname "$0")/.."
J=app/src/main/jniLibs/arm64-v8a; mkdir -p "$J" tools/natives
command -v patchelf >/dev/null || { echo "patchelf is required" >&2; exit 1; }
TAR="unidatum-$V-android-arm64.tar.gz"
if [ ! -f "tools/natives/$TAR" ]; then
  if [ -f "$LOCAL_DIST/$V/$TAR" ]; then cp "$LOCAL_DIST/$V/$TAR" tools/natives/
  else gh release download "$V" -R "$REPO" -p "$TAR" -D tools/natives --clobber; fi
fi
tar xzf "tools/natives/$TAR" -C tools/natives --strip-components=1 "unidatum-$V-android-arm64/unidatum"
